...
<body>
	<!-- html 파일과 JSP 파일의 다른점 
		 JSP page 지시자 : java 프로그래밍 -> 웹 애플리케이션 서버(WAS) -> 서버가 컴파일 -> html 문서 응답 
		 ...
		 -->
	<!-- 이 소스파일에 자바 코드를 작성 -> 스크립트릿(코드 조각) -->
	<%
		String message = "hello? 그 동안 잘 있었니 자바야";
		LocalDate now = LocalDate.now(); // java.lang 패키지 아닌 클래스 import
	%>
	<h4>I am JSP</h4>
	<ul>
		<li><%=now%></li>
		<!-- 자바코드의 변수값(표현식) 출력해 보기 -->
		<li><%=message%></li>
		<!-- 바로 ${message} 기호 사용할 예정 -->
		<%
